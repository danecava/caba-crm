...
// Re-rate every policy's commission % to match the current comp grid (by carrier+product).
function rerateBook() {
  const norm = (s) => String(s || '').trim().toLowerCase().replace(/\s+/g, ' ');
  const grid = db.prepare('SELECT carrier, product, rate FROM comp_grid').all();
  const map = new Map();
  grid.forEach((g) => map.set(norm(g.carrier) + '|' + norm(g.product), g.rate));
  const pols = db.prepare("SELECT id, carrier, product, comm_pct FROM policies WHERE carrier IS NOT NULL").all();
  let matched = 0, changed = 0; const unmatched = [];
  db.exec('BEGIN');
  try {
    const upd = db.prepare('UPDATE policies SET comm_pct=? WHERE id=?');
    for (const p of pols) {
      const rate = map.get(norm(p.carrier) + '|' + norm(p.product));
      if (rate != null) { matched++; if (p.comm_pct !== rate) { upd.run(rate, p.id); changed++; } }
      else unmatched.push(`${p.carrier} / ${p.product}`);
    }
    db.exec('COMMIT');
  } catch (e) { try { db.exec('ROLLBACK'); } catch {} throw e; }
  return { policies: pols.length, matched, changed, unmatched: [...new Set(unmatched)] };
}

module.exports = { wipeDemo, importData, listUsers, createUser, updateUser, setActive, resetPassword, rerateBook };
